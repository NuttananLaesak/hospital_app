...
class EditNihss12 extends StatefulWidget {
  final int patientID;

  const EditNihss12({Key? key, required this.patientID}) : super(key: key);

  @override
  State<EditNihss12> createState() => _EditNihss12State();
}

class _EditNihss12State extends State<EditNihss12> {
  ...
  Future<void> _loadPatientData() async {
    ...
    if (result.isNotEmpty) {
      setState(() {
        _oldselectedScore = result.first['SelectedScore12'];
  ...
  Future<void> _updatePatientData(int score, String text) async {
    ...
    String updateQuery = "UPDATE Patient SET "
        "SelectedScore12 = $_selectedScore, "
        "SelectedText12 = '$_selectedText', "
  ...
  Widget build(BuildContext context) {
    ...
    List<String> _questions = [
      ...
      "สูญเสียการรับความรู้สึกในระดับรุนเเรง",
    ];
