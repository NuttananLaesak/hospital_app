...
class EditNihss1 extends StatefulWidget {
  final int patientID;

  const EditNihss1({Key? key, required this.patientID}) : super(key: key);

  @override
  State<EditNihss1> createState() => _EditNihss1State();
}

class _EditNihss1State extends State<EditNihss1> {
  ...
  Future<void> _loadPatientData() async {
    ...
    if (result.isNotEmpty) {
      setState(() {
        _oldselectedScore = result.first['SelectedScore1'];
  ...
  Future<void> _updatePatientData(int score, String text) async {
    ...
    String updateQuery = "UPDATE Patient SET "
        "SelectedScore1 = $_selectedScore, "
        "SelectedText1 = '$_selectedText', "
  ...
  Widget build(BuildContext context) {
    ...
    List<String> _questions = [
      ...
      "ไม่รู้สึกตัว ต้องกระตุ้นซ้ำหรือทำให้เจ็บ",
