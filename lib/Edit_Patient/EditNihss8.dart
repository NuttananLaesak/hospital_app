...
class EditNihss8 extends StatefulWidget {
  final int patientID;

  const EditNihss8({Key? key, required this.patientID}) : super(key: key);

  @override
  State<EditNihss8> createState() => _EditNihss8State();
}

class _EditNihss8State extends State<EditNihss8> {
  ...
  Future<void> _loadPatientData() async {
    ...
    if (result.isNotEmpty) {
      setState(() {
        _oldselectedScore = result.first['SelectedScore8'];
  ...
  Future<void> _updatePatientData(int score, String text) async {
    ...
    String updateQuery = "UPDATE Patient SET "
        "SelectedScore8 = $_selectedScore, "
        "SelectedText8 = '$_selectedText', "
  ...
  Widget build(BuildContext context) {
    ...
    String questionText =
        "ข้อที่ 5b กำลังของกล้ามเนื้อเเขนขวา\n(Motor Right Arm)";
