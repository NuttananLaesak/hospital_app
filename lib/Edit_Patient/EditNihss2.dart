...
class EditNihss2 extends StatefulWidget {
  final int patientID;

  const EditNihss2({Key? key, required this.patientID}) : super(key: key);

  @override
  State<EditNihss2> createState() => _EditNihss2State();
}

class _EditNihss2State extends State<EditNihss2> {
  ...
  Future<void> _loadPatientData() async {
    ...
    if (result.isNotEmpty) {
      setState(() {
        _oldselectedScore = result.first['SelectedScore2'];
  ...
  Future<void> _updatePatientData(int score, String text) async {
    ...
    String updateQuery = "UPDATE Patient SET "
        "SelectedScore2 = $_selectedScore, "
        "SelectedText2 = '$_selectedText', "
  ...
  Widget build(BuildContext context) {
    ...
    List<String> _questions = [
      ...
      "ไม่สามารถตอบได้หรือตอบผิด 2 ข้อ"
    ];
