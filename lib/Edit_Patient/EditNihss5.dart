...
class EditNihss5 extends StatefulWidget {
  final int patientID;

  const EditNihss5({Key? key, required this.patientID}) : super(key: key);

  @override
  State<EditNihss5> createState() => _EditNihss5State();
}

class _EditNihss5State extends State<EditNihss5> {
  ...
  Future<void> _loadPatientData() async {
    ...
    if (result.isNotEmpty) {
      setState(() {
        _oldselectedScore = result.first['SelectedScore5'];
  ...
  Future<void> _updatePatientData(int score, String text) async {
    ...
    String updateQuery = "UPDATE Patient SET "
        "SelectedScore5 = $_selectedScore, "
        "SelectedText5 = '$_selectedText', "
  ...
  Widget build(BuildContext context) {
    ...
    String questionText = "ข้อที่ 3 การมองเห็น\n(Visual Fields)";
    List<String> _questions = [
      "ลานสายตาปกติ",
      "ลานสายตาผิดปกติบางส่วน",
      "ลานสายตาผิดปกติครึ่งซีก",
      "มองไม่เห็นทั้งสองตา (ตาบอด)"
    ];
